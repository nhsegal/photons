...
function draw() {
  ...
  if (state !=0){
    ...
    if (menu.value() != "Double Slit"){
      ...
      stroke(0);
      for (let i = 0; i < pathNumber; i++) {
        drawArrow(contactPoints[i].x, spacing + midPoint + 6, contactPoints[i].phase, 20);
      }
      /*
      for (let i = floor(pathNumber/2); i < pathNumber; i++) {
        drawArrow(5*contactPoints[i].x-1.6*effectiveWidth, spacing + midPoint + 6, contactPoints[i].phase);
      }
    */
    }
  }
